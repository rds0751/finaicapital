...
from .models import random_id, MetatraderTradeResponse, MarketTradeOptions, StopOptions, MarginOrder, \
...
class MetaApiConnectionInstance(ABC):
    ...
    def __init__(self, websocket_client: MetaApiWebsocketClient, metaapi_connection: MetaApiConnection):
        ...
        self._metaApiConnection = metaapi_connection
        self._instanceId = random_id()
    ...
    def calculate_margin(self, order: MarginOrder):
        ...
        """
        self._check_is_connection_active()
        return self._websocketClient.calculate_margin(self._metaApiConnection.account.id,
                                                      self._metaApiConnection.application,
                                                      self._metaApiConnection.account.reliability, order)
    ...
    def _trade(self, request: dict):
        return self._websocketClient.trade(self.account.id, request, self._metaApiConnection.application,
                                           self._metaApiConnection.account.reliability)
